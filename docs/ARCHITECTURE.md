...
### По Connection/Session в Artemis
...
✅ Транзакции (если будут) изолированы по Session
❌ Нужно thread-safe управление созданием Sessions
```
...
sharedConnection.start(); // Один раз!

// В каждом virtual thread (акторе)
...
✅ Настраиваемый (null handling, date formats, etc)
✅ Быстрый
...
└──────┬──────┘         └──────┬───────┘
       ...
       └───────┬───────────────┘
               ↓
        ┌──────────────┐
        │   MVStore    │ ← сохранили incoming message
        │ incoming_msg │
        └──────────────┘
               │
               ↓ (traceId в Artemis)
        ...
        └──────┬───────┘
               ↓
        ┌──────────────┐
        │ Incoming     │ ← пул акторов (virtual threads)
        │ Actor Pool   │
        └──────┬───────┘
               │ читает из MVStore, трансформирует
               ↓
        ┌──────────────┐
        │   MVStore    │ ← сохранили domain message
        │ domain_msg   │
        └──────────────┘
               │
               ↓ (traceId в Artemis)
        ...
        └──────┬───────┘
               ↓
        ┌──────────────┐
        │ GameEngine   │ ← пул акторов
        │ Actor Pool   │ ← считает символы 😄
        └──────┬───────┘
               │
               ↓
        ┌──────────────┐
        │   MVStore    │ ← сохранили reply
        │  reply_msg   │
        └──────────────┘
               │
               ↓ (traceId в Artemis)
        ...
        └──────┬───────┘
               ↓
        ┌──────────────┐
        │ Outgoing     │ ← пул акторов
        │ Actor Pool   │ ← пишет в outbox
        └──────┬───────┘
               │
               ↓
        ┌──────────────┐
        ...
        ┌──────────────┐
        │  Outbox      │
        │  Processor   │ → отправка в Telegram/Console
        └──────────────┘
```
...
```
~/.mz4d/
  ├── logs/           ← logback пишет сюда
  ├── data/
  │   ├── mvstore/    ← game.mv.db
  │   └── artemis/    ← journal/bindings/etc
  └── config/
      └── application.conf
```
